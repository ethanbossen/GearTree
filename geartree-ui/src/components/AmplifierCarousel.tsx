...
function AmplifierCarousel({ amplifiers }: AmplifierCarouselProps) {
  ...
  if (!amplifiers || amplifiers.length === 0) {
    return <p className="text-gray-500">No amplifiers found.</p>;
  }
  ...
  return (
    <div className="relative w-full p-4 group min-w-0 overflow-hidden">
      <AnimatePresence mode="wait">
        <motion.div
          key={amplifiers[currentIndex].id}
        ...
        >
          {/* Amp Card */}
          <Link
            to={`/amplifiers/${amplifiers[currentIndex].id}`}
          ...
          >
            {amplifiers[currentIndex].photoUrl && (
              <img
                src={amplifiers[currentIndex].photoUrl}
                alt={amplifiers[currentIndex].name}
            ...
            <p className="text-sm text-gray-600 italic">
              {amplifiers[currentIndex].yearStart} -{" "}
              {amplifiers[currentIndex].yearEnd || "Present"}
            </p>
            <p className="text-gray-700 text-center line-clamp-2 min-h-[3rem]">
              {amplifiers[currentIndex].summary}
            </p>
          </Link>
        </motion.div>
      </AnimatePresence>
